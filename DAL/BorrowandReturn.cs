...
namespace YueDuLibrary.DAL
{
	...
	public partial class BorrowandReturn
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "BorrowandReturn"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from BorrowandReturn");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into BorrowandReturn(");
			strSql.Append("ReaderID,BookCode,BorrowTime,BackTime)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update BorrowandReturn set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from BorrowandReturn ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		public YueDuLibrary.Model.BorrowandReturn GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,ReaderID,BookCode,BorrowTime,BackTime from BorrowandReturn ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public YueDuLibrary.Model.BorrowandReturn DataRowToModel(DataRow row)
		{
			YueDuLibrary.Model.BorrowandReturn model=new YueDuLibrary.Model.BorrowandReturn();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,ReaderID,BookCode,BorrowTime,BackTime ");
			strSql.Append(" FROM BorrowandReturn ");
			if(strWhere.Trim()!="")
			{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select bookName,bookImg,BookInfo.bookCode,BorrowTime,BackTime from BookInfo inner join BorrowandReturn on  BookInfo.bookCode=BorrowandReturn.BookCode ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where" + strWhere);
            }
            strSql.Append(" order by BorrowandReturn.BorrowTime desc" );
            return DbHelperSQL.Query(strSql.ToString());
        }
        ...
        public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,ReaderID,BookCode,BorrowTime,BackTime ");
			strSql.Append(" FROM BorrowandReturn ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from BorrowandReturn T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "BorrowandReturn";
